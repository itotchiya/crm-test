  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm">
      ...
      <div className="space-y-3">
        {deals.map((deal) => (
          <div key={deal.id} className="flex items-center justify-between p-3 rounded-lg border border-slate-100 hover:border-slate-200 transition-colors">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-medium text-slate-900">{deal.company}</p>
                <p className="text-xs text-slate-500">{deal.contact}</p>
              </div>
            </div>
            <div className="text-right">
              <p className="text-sm font-semibold text-slate-900">{deal.value}</p>
              <span className={cn("px-2 py-0.5 rounded-full text-xs font-medium", stageColors[deal.stage] || "bg-slate-100 text-slate-700")}>
                {deal.stage}
              </span>
            </div>
          </div>
