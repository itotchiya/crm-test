  ...
  return (
    <div className="space-y-6">
      <div className="mb-6">
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Deep insights into your sales performance.</p>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {stats.map((stat) => (
          <div key={stat.label} className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 p-5 shadow-sm">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 p-5 shadow-sm">
          <h3 className="text-base font-semibold text-slate-900 dark:text-white mb-1">Revenue & Deals Trend</h3>
          <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">Monthly performance overview</p>
          <div className="h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={monthlyPerformance} margin={{ top: 5, right: 5, left: -20, bottom: 0 }}>
                ...
                <Tooltip contentStyle={{ backgroundColor: "#1e293b", border: "1px solid #334155", borderRadius: "8px", boxShadow: "0 4px 6px -1px rgb(0 0 0 / 0.3)", color: "#fff" }} />
        ...
        <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 p-5 shadow-sm">
          <h3 className="text-base font-semibold text-slate-900 dark:text-white mb-1">Revenue Growth</h3>
          <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">Cumulative revenue trajectory</p>
          <div className="h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={monthlyPerformance} margin={{ top: 5, right: 5, left: -20, bottom: 0 }}>
                ...
                <Tooltip contentStyle={{ backgroundColor: "#1e293b", border: "1px solid #334155", borderRadius: "8px", boxShadow: "0 4px 6px -1px rgb(0 0 0 / 0.3)", color: "#fff" }} formatter={(v: number) => [`$${v.toLocaleString()}`, "Revenue"]} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 p-5 shadow-sm">
          <h3 className="text-base font-semibold text-slate-900 dark:text-white mb-1">Team Performance</h3>
          <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">Individual contributor metrics</p>
            ...
            <table className="w-full">
              ...
              <tbody>
                {teamPerformance.map((member) => (
                  <tr key={member.id} className="border-b border-slate-50 dark:border-slate-800/50 hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                    <td className="py-3 pr-4">
                      <div className="flex items-center gap-3">
                        <div className="w-9 h-9 rounded-full bg-gradient-to-br from-indigo-100 to-purple-100 dark:from-indigo-900 dark:to-purple-900 flex items-center justify-center text-sm font-semibold text-indigo-700 dark:text-indigo-300">
                          {member.avatar}
                        </div>
                        <div>
                    ...
                    <td className="py-3 pr-4 text-sm font-semibold text-slate-900 dark:text-white">{member.deals}</td>
                    <td className="py-3 pr-4 text-sm font-semibold text-slate-900 dark:text-white">${(member.revenue / 100).toLocaleString()}</td>
                    <td className="py-3 pr-4">
                      <div className="flex items-center gap-2">
                        <div className="w-20 h-1.5 bg-slate-100 dark:bg-slate-700 rounded-full overflow-hidden">
                          <div className="h-full bg-indigo-500 rounded-full" style={{ width: `${member.conversion * 3}%` }} />
                        </div>
                        <span className="text-xs text-slate-500 dark:text-slate-400">{member.conversion}%</span>
                      </div>
                    </td>
                    <td className="py-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-slate-500 dark:text-slate-400">{member.target}%</span>
                      </div>
                    </td>
                  </tr>
                ))}
        ...
        <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 p-5 shadow-sm">
          <h3 className="text-base font-semibold text-slate-900 dark:text-white mb-1">Conversion by Source</h3>
          <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">Lead to customer conversion</p>
